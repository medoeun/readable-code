...
public class MinesweeperGame {
    ...
    private static final String[][] BOARD = new String[BOARD_ROW_SIZE][BOARD_COL_SIZE]; //가로세로 크기
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (doesUserChooseToPlantFlag(userActionInput)) {
                ...
                checkIfGameIsOver();
            } else if (doesUserChoooseToOpenCell(userActionInput)) {
                if (isLandMineCell(selectedRowIndex, selectedColIndex)) {
                    BOARD[selectedRowIndex][selectedColIndex] = LAND_MINE_SIGN;
                    changeGameSatusToLose();
                    continue;
                } else {
    ...
    private static void changeGameSatusToLose() {
        gameStatus = -1;
    }
    ...
    private static boolean doesUserChoooseToOpenCell(String userActionInput) {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                ...
                if (!isLandMineCell(row, col)) {
                    ...
                    NEARBY_LAND_MINE_COUNTS[row][col] = count; // count: // 현재 좌표 기준 주변 8칸에 지뢰가 몇 개인지
                    continue;
                }
